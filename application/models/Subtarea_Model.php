...
class Subtarea_Model extends CI_Model {
  ...
  public function getTareaSubtareas($tareaID, $eliminadas = true){
    #$eliminadas omite a las tareas eliminadas si el valor pasado es true
    $this->db->select('
      usuario.nombres,
      usuario.apellidos,
      estado_tarea.nombre as estado,
      subtarea.*
    ')
    ->from('subtarea')
    ->join('tarea', 'tarea.id = subtarea.tarea_id')
    ->join('estado_tarea', 'estado_tarea.id = tarea.estado_tarea_id')
    ->join('usuario', 'usuario.id = subtarea.empleado_id')
  ...
  public function getSubtareas($empleadoID, $tareaID = NULL, $eliminadas = NULL){
    #$eliminadas omite a las tareas eliminadas si el valor pasado es true
    $this->db->select('campania.id as campania_id,
      campania.nombre as nombre_campania,
      red_semantica.id as red_id,
  ...
  public function registrarSubtarea($tareaID, $empleadoID, $descripcion, $fechaEntrega){
    ...
    $data = [
      ...
      '_create' => $fechaRegistro,
      '_update' => $fechaRegistro
    ];
  ...
  public function editarSubtarea($subtareaID, $descripcion, $fechaEntrega, $estadoTareaID=1){
  ...
  public function eliminarSubtarea($subtareaID){
    ...
    $data = [
      '_erase' => $fechaEliminacion,
      '_update' => $fechaEliminacion
    ];
